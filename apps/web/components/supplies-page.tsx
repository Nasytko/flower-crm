...
function isOverdueUnpaid(row: {
  ...
  if (row.isPaid || row.status === SupplyStatus.CANCELLED || !row.paymentDueDate) return false;
  return row.paymentDueDate < todayIso();
}
              ...
              <table className="w-full min-w-[980px] text-left text-sm">
                <thead>
                  <tr className="border-b border-border text-[11px] uppercase tracking-[0.08em] text-muted-foreground">
                    ...
                    <th className="px-3 py-3 font-medium">Оплата</th>
                    <th className="px-3 py-3 font-medium">Крайняя дата</th>
                ...
                <tbody>
                  {items.map((row) => {
                    const overdue = isOverdueUnpaid(row);
                    return (
                  ...
                  >
                    ...
                    <div className="mt-2 space-y-1">
                      ...
                      <StatRow
                        label="Крайняя дата"
                        value={
                          <span className={cn(overdue && 'font-semibold text-danger-fg')}>
                            {row.paymentDueDate ?? '—'}
                            {overdue ? ' · просрочено' : ''}
                          </span>
                        }
...
function PaidBadge({
  ...
  return (
    <span
      className={cn(
        'inline-flex shrink-0 rounded-full px-3 py-1 text-xs font-semibold',
        isPaid ? 'bg-success-soft text-success-fg' : 'bg-warn-soft text-warn-fg',
      )}
    >
